...
Accounts arrived with v0.10: an email and a password, Argon2id hashes, and sessions as rows in the database so that signing out actually ends one ([ADR 0012](https://github.com/lacodda/lyrid/blob/main/docs/adr/0012-accounts-with-a-password.md)). An account remembers your marker and where you left the sky. Nothing is gated behind it: the map, the card and the search answer a visitor exactly as they answer an account, and a saved camera is dropped rather than trusted when the sky has been rebuilt under it.

v0.11 opened the doors. The address is confirmed by a letter and a lost password can be recovered by one — and a stand with no route to the internet writes those letters to its log instead of failing, link and all, which is a supported configuration rather than a fallback. The door no longer asks which mode you want: only the creative one is built, so every account starts there and the choice arrives with the fog, offered once. There is a privacy charter at `/charter`, and both of its promises are pressable rather than described: the export hands back the rows themselves, and the deletion is one statement and the schema's cascades — immediate, total, and not a queue. Usage is counted as counters per mechanic per day, never as events, so there is nothing to join back to a person. Tiles are cacheable for a day and the neighbouring zoom levels are fetched while you are still looking at the current one, which makes the second visit and the next zoom both instant. A link to lyrid unfurls into a picture of the sky, and a single star can be quoted into someone else's page as an iframe.

The fog of war comes next. Watch this repository.
